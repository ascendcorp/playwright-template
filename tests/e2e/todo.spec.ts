...
const todoNames = ['buy some cheese', 'feed the cat', 'book a doctors appointment']

const [firstTodoName, secondTodoName, thirdTodoName] = todoNames
...
test.describe('New Todo', () => {
  test('should allow me to add todo items', async ({ page }) => {
    ...
    await expect(page.locator('.view label')).toHaveText([firstTodoName])
    ...
    await expect(page.locator('.view label')).toHaveText([firstTodoName, secondTodoName])
  ...
  test('should append new items to the bottom of the list', async ({ page }) => {
    ...
    await expect(page.locator('.view label')).toHaveText(todoNames)
...
test.describe('Item', () => {
  test('should allow me to mark items as complete', async ({ page }) => {
    // Create two items.
    for (const item of todoNames.slice(0, 2)) {
      await page.locator('.new-todo').fill(item)
      await page.locator('.new-todo').press('Enter')
    }
  ...
  test('should allow me to edit an item', async ({ page }) => {
    ...
    await expect(todoItems).toHaveText([firstTodoName, 'buy some sausages', thirdTodoName])
...
test.describe('Editing', () => {
  ...
  test('should cancel edits on escape', async ({ page }) => {
    ...
    await expect(todoItems).toHaveText(todoNames)
  })
})
